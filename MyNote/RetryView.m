...
+ (RetryView *)retryViewWithText:(NSString *)text buttonText:(NSString *)buttonText target:(id)target action:(SEL)action{
    ...
    UIView *retryView = [[UIView alloc] initWithFrame:CGRectMake(0, 64, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 64)];
    ...
    [retryView addSubview:blankLabel];
    ...
    CGFloat writeButtonY = (retryView.frame.size.height - writeButton.frame.size.height)/2 + 60;
    ...
    [retryView addSubview:writeButton];
